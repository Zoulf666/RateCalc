...
class Custom:
    ...
    def delete_custom(self, custom_name):
        SQL = """
        DELETE FROM Custom WHERE custom_name=?
        """
...
class CustomDetail():
    ...
    def find_custom_detail(self, custom_id, provice_name):
        ...
        WHERE custom=? AND provice_name=?;
        """
        self.cur.execute(SQL, (custom_id, provice_name))
        provice_id = self.cur.fetchone()
        return provice_id[0] if provice_id else 0
...
if __name__ == '__main__':
    ...
    # custom_detail.add_custom_detail(1, '湖南', 1.0, 1.0, 1.0)
    # custom_detail.close()
    # custom = Custom()
    # custom_id = custom.add_custom('zoulf')
    # print(custom_id)
    pass
