...
def edit_provice_info(custom_name, btn_confirm):
    ...
    if provice_root.winfo_screenheight() > 768:
        provice_root.geometry(utils.center_root(provice_root, 400, 840))
    else:
        provice_root.geometry(utils.center_root(provice_root, 1300, 200))
    ...
    provice_info = custom_detail.fetch_custom_detail(custom_id)
    l1 = tkinter.Label(provice_root, text='省份', font=('Helvetica', '9', 'normal'))
    l2 = tkinter.Label(provice_root, text='首重重量(kg)', font=('Helvetica', '9', 'normal'))
    l3 = tkinter.Label(provice_root, text='首重价格(元)', font=('Helvetica', '9', 'normal'))
    l4 = tkinter.Label(provice_root, text='续重价格(元)', font=('Helvetica', '9', 'normal'))
    for index, provice in enumerate(provices):
        ...
        tmp = []
        provice_label = tkinter.Label(provice_root, text=provice, font=('Helvetica', '9', 'normal'))
        f_weight = tkinter.Entry(provice_root, width=5, textvariable=f_w_text, font=('Helvetica', '9', 'normal'))
        f_price = tkinter.Entry(provice_root, width=5, textvariable=f_p_text, font=('Helvetica', '9', 'normal'))
        n_price = tkinter.Entry(provice_root, width=5, textvariable=n_p_text, font=('Helvetica', '9', 'normal'))
    ...
    btn_cancel = tkinter.Button(provice_root, text='取消', command=provice_root.destroy)
    if provice_root.winfo_screenheight() > 768:
        ...
        btn_save.grid(row=len(provices) + 2, column=0)
        btn_cancel.grid(row=len(provices) + 2, column=1, columnspan=3, sticky=tkinter.E)
    else:
        l1.grid(row=0, column=0)
